...
const generateAccessAndRefreshTokens = async(userId) => {
    try {
        ...
        user.refreshToken = refreshToken   // database me save karwa diya
        await user.save({ validateBeforeSave: false }) // save

        return {accessToken, refreshToken}   // accessToken and refreshToken generated.
...
const loginUser = asyncHandler(async (req, res) => {
    ...
    // IMPORTANT
    // User -> mongoose ka object h
    // user -> this is my user jo database se wapas liya h (uska instance liya h) to iske pass mere banaye hue methods hote h.
    if(!user){
    ...
    const options = {
        httpOnly: true, // this cookie is only modifiable by the server not by the frontend.
        secure: true   // this cookie is only modifiable by the server not by the frontend.
    }
...
const updateAccountDetails = asyncHandler(async(req, res) => {
    ...
    const user = await User.findByIdAndUpdate(
        req.user?._id,
        {
            $set: {
                fullname,
                email: email    // both above one and the below one will have same function.
            }
        },
        {new: true}
    ).select("-password")
